...
 
readonly ORGANISATION="arxan"
readonly DOMAIN="arxanfintech.com"
readonly ADMIN_PASSWORD="admin"
readonly BASE_DN="arxanfintech.com"
readonly REPLICATION="true"
readonly CNNAME="arxanfintech"
readonly DC="com"
 
docker run --hostname ldap.example.org --name ldap-service -v /data/ldap/data:/var/lib/ldap -v /data/ldap/sldap:/etc/ldap/slapd.d \
-p 389:389 \
-p 636:636 \
...
--env LDAP_REPLICATION=${REPLICATION} --detach osixia/openldap:1.1.11
LDAP_IP=$(docker inspect -f "{{ .NetworkSettings.IPAddress }}" ldap-service)
 
#Create the second ldap server, save the container id in LDAP2_CID and get its IP:
docker run --hostname ldap2.example.org --name ldap-service2 \
 -v /data/ldap2/data:/var/lib/ldap -v /data/ldap2/sldap:/etc/ldap/slapd.d \
--env LDAP_ORGANISATION=${ORGANISATION}  \
--env LDAP_DOMAIN=${DOMAIN} \
--env LDAP_ADMIN_PASSWORD=${ADMIN_PASSWORD} \
--env LDAP_BASE_DN=${BASE_DN} \
